...
def home(request):
	...
	serid=models.Service.objects.all().order_by('-id')
	return render(request, 'home.html',{'banners':banners,'services':services,'gimgs':gimgs,'sid':serid})
...
def pay_success(request):
	...
	msg.send()
	plans=models.Subscription.objects.filter(user=request.user).count()
	if plans>1:
		models.Subscription.objects.filter(user=request.user)[0].delete()
...
def user_dashboard(request):
	#l=models.Subscription.objects.get(user=request.user)
...
def dietplan(request):
	plans = models.DietPlan.objects.all()
	cal= models.Plan.objects.all()
	return render(request, 'dietplan.html', {'plans':plans,'cals':cal})
